    ...
    >
      ...
      <div class="accordeon" :class="{ 'p-2': options.showMap }">
        <div class="aitem" v-if="options.showArrivals">
          ...
          >
            <table class="table table-dark shadow-none mb-0">
              <thead>
                <tr>
                  <th scope="col" v-if="options.columns.date">
                    {{ $t("Date") }}
                  </th>
                  <th
                    scope="col"
                  ...
                  <th scope="col" v-if="options.columns.flightnumber">
                    {{ $t("Flight") }}
                  </th>
                  <th
                    scope="col"
              ...
              <tbody>
                <tr v-for="arrival in lastArrivals" :key="arrival.key">
                  ...
                  <th class="text-center" v-if="options.columns.time">
                    {{ asZoneTime(arrival.time, "UTC", arrival.date) }}
                  </th>
                  ...
                  <td
                    v-if="
                      airport_types[arrival.departure] && options.columns.fromto
                    "
                  >
                  ...
                  <td
                    v-if="
                      !airport_types[arrival.departure] &&
                      options.columns.fromto
                    "
                  ></td>
                  ...
                  <td class="text-end" v-if="options.columns.rate">
                    <span
                      v-if="
                        remark(arrival)['text'] == 'SCHEDULED' && arrival.rates
                      "
                    >
                ...
                <tr
                  v-if="
                    max_entries < this.arrivals.length &&
                    options.columns.morebutton
                  "
                  class="load-more"
                >
                  <td colspan="10" class="text-center">
                    <div @click="loadMore()">
                      {{ $t("load more") }} ({{ max_entries }}/{{
            ...
            <table class="table table-dark shadow-none mb-0">
              ...
              <tbody>
                <tr v-for="departure in lastDepartures" :key="departure.key">
                  ...
                  <td
                    v-if="
                      airport_types[departure.arrival] && options.columns.fromto
                    "
                  >
                  ...
                  <td
                    v-if="
                      !airport_types[departure.arrival] &&
                      options.columns.fromto
                    "
                  ></td>
                  <td v-if="options.columns.flightnumber">
                  ...
                  <td class="text-end" v-if="options.columns.rate">
                    <span
                      v-if="
                        remark(departure)['text'] == 'SCHEDULED' &&
                        departure.rates
                      "
                      >{{
                        departure.rates.basic_adult_oneway_withtaxes
                      }}
                      €</span
                    >
                  </td>
                ...
                <tr
                  v-if="
                    max_entries < this.departures.length &&
                    options.columns.morebutton
                  "
                  class="load-more"
                >
        ...
        <div class="aitem" v-if="options.showRealtimeTable">
          ...
          <div
            class="table-responsive realtime mb-2"
            :style="
              'overflow-y: auto; overflow-x: hidden; height:' +
              options.tableheight
            "
  ...
  methods: {
    ...
    $t: function (mess) {
      ...
      if (this.options.messages)
        messages = { ...messages, ...this.options.messages };
    ...
    async fetchSchedules() {
      try {
        ...
        const where =
          "and(or(smetadata.fromdestination.eq." +
          this.options.filters.airport +
        ...
        const airport = this.options.filters.airport
          ? this.options.filters.airport
          : "BZO";
        ...
        data = data.filter((json) => {
          ...
          const isAirport =
            json.departure == airport || json.arrival == airport;

          if (this.options.nations.length > 0) {
            const excludes = this.options.nations.filter(
              (o) => o.indexOf("!") != -1
            );
          ...
          if (this.options.airports.length > 0) {
            const excludes = this.options.airports.filter(
              (o) => o.indexOf("!") != -1
            );
            // excludes > includes
            if (excludes.length > 0) {
              excludes.forEach((exc) => {
                exc = exc.substring(1);
                if (json.departure == exc || json.arrival == exc)
                  validAirport = false;
              });
            } else {
              if (
